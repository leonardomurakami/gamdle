...
var WHEEL_STEP = 360 / EUROPEAN_WHEEL_ORDER.length;
...
// frontend/reveals/roulette.js
var point = (radius, angle) => `${160 + Math.cos(angle) * radius},${160 + Math.sin(angle) * radius}`;
function renderWheelNumbers(group) {
  group.innerHTML = EUROPEAN_WHEEL_ORDER.map((number2, index) => {
    ...
    const className = number2 === 0 ? "green-pocket" : RED_NUMBERS.has(number2) ? "red-pocket" : "black-pocket";
    return `<path class="${className}" d="${path}"></path><text x="${x.toFixed(2)}" y="${y.toFixed(2)}" transform="rotate(${(middle * 180 / Math.PI + 90).toFixed(2)} ${x.toFixed(2)} ${y.toFixed(2)})">${number2}</text>`;
  }).join("");
}
...
// frontend/reveals/cards.js
var SUITS = { hearts: "\u2665", diamonds: "\u2666", clubs: "\u2663", spades: "\u2660" };
var RANKS = { 1: "A", 11: "J", 12: "Q", 13: "K" };
function createCardReveal({ controller, sound: sound2 }) {
  ...
  function populate(resolution) {
    ...
    document.querySelectorAll("[data-card-rank]").forEach((element) => {
      element.textContent = rank;
    });
    document.querySelectorAll("[data-card-suit]").forEach((element) => {
...
// frontend/reveals/dice.js
function createDiceReveal({ controller, sound: sound2 }) {
...
// frontend/reveals/slots.js
var REEL_CYCLES = 6;
function buildStrip(reel) {
  const symbols = Array.from({ length: REEL_CYCLES + 2 }, () => SLOT_SYMBOL_ORDER).flat();
  reel.innerHTML = `<div class="reel-strip" aria-hidden="true">${symbols.map((symbol) => `<span data-symbol="${symbol}">${SLOT_SYMBOL_GLYPHS[symbol]}</span>`).join("")}</div>`;
}
function createSlotsReveal({ controller, sound: sound2 }) {
...
function createGameAnimations(sound2) {
  const controller = createRevealController(document.querySelector("#skip-animation"));
  const reveals = {
    wheel: createRouletteReveal({ controller, sound: sound2 }),
    cards: createCardReveal({ controller, sound: sound2 }),
    dice: createDiceReveal({ controller, sound: sound2 }),
    slots: createSlotsReveal({ controller, sound: sound2 })
  };
...
// frontend/sound.js
function createSound(getEnabled) {
  let context = null;
  ...
  function tone(frequency, duration = 0.08, offset = 0, type = "sine", gain = 0.035) {
    ...
    volume.gain.exponentialRampToValueAtTime(1e-4, context.currentTime + offset + duration);
  ...
  return { initialize: initialize2, play, tone };
}
...
var format = (number2) => new Intl.NumberFormat("en-US").format(number2);
var percent2 = (value) => {
...
function renderBetSlip() {
  ...
  $("#chance").textContent = percent2(selection.probability);
...
function renderHistory() {
  ...
  history2.innerHTML = wagers.slice().reverse().map((wager) => `
...
function renderResults() {
  ...
  const privacyNote = state.mode === "guest" ? " This guest result was not saved or added to the standings." : "";
  $("#result-summary").textContent = (difference >= 0 ? `You finished ${format(difference)} points ahead after ${state.run.move} moves.` : `You finished ${format(Math.abs(difference))} points below the opening bankroll after ${state.run.move} moves.`) + privacyNote;
...
$("#login-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    message.innerHTML = body.developmentLink ? `${body.message}<br><a href="${body.developmentLink}">Open development login link</a>` : body.message;
  } catch (error) {
    showToast(error.message);
  } finally {
...
$("#bet-controls").addEventListener("click", (event) => {
  ...
  if (button.dataset.threshold) {
    ...
    const selection = state.rules.games.cards.bets.find((bet) => bet.bet.threshold === threshold && bet.bet.direction === validDirection);
    state.selection = selection;
    renderControls();
  }
...
$("#change-email-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    message.hidden = false;
    message.innerHTML = body.developmentLinks ? `${body.message}<br>${body.developmentLinks.map((link, index) => `<a href="${link}">Open ${index ? "new" : "current"} email link</a>`).join("<br>")}` : body.message;
  } catch (error) {
    showToast(error.message);
  }
});
$("#delete-button").addEventListener("click", async () => {
  ...
  try {
    ...
    const message = $("#account-message");
    message.hidden = false;
    message.innerHTML = body.developmentLink ? `${body.message}<br><a href="${body.developmentLink}">Open development deletion link</a>` : body.message;
  } catch (error) {
    showToast(error.message);
  }
});
